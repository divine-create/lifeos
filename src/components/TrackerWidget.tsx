    ...
    <div className="flex flex-col h-full w-full">
      <div className="flex-1 flex flex-col justify-between overflow-x-auto pb-4">
          ...
          <div className="min-w-[600px] pr-8">
            ...
            <div className="space-y-1">
              {trackers.map((tracker) => {
                return (
                  <div key={tracker.id} className="group relative flex items-center justify-between rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800/50 dark:bg-zinc-800/50 p-2 transition border border-transparent hover:border-gray-100 dark:hover:border-zinc-700 dark:border-zinc-800">
                            ...
                            <form key={i} action={async (formData) => { await logTrackerEntry(formData); toast.success("Saved"); }} className="w-10 flex justify-center">
                              ...
                              <button type="submit" className={`flex h-8 w-8 items-center justify-center rounded-md border transition-all ${
                                isActive 
                                  ...
                                  : "border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50/50 text-transparent hover:border-gray-400 hover:text-gray-300 dark:hover:text-zinc-500"
                              }`}>
                            ...
                            <div key={i} className="w-10 flex justify-center relative group/input">
                              {tracker.type === "RATING" ? (
                                <form action={async (formData) => { await logTrackerEntry(formData); toast.success("Saved"); }} className="flex flex-col items-center">
                                  <input type="hidden" name="trackerId" value={tracker.id} />
                                  <input type="hidden" name="date" value={dateIso} />
                                  <select name="value" defaultValue={log?.value || ""} onChange={(e) => e.target.form?.requestSubmit()} className="h-8 w-10 text-xs border border-gray-300 dark:border-zinc-600 rounded text-center appearance-none bg-white dark:bg-zinc-900 cursor-pointer focus:ring-1 focus:ring-blue-500 pl-1">
                                    <option value="">-</option>
                                ...
                                <form action={async (formData) => { await logTrackerEntry(formData); toast.success("Saved"); }} className="flex flex-col items-center">
                                  ...
                                  <input 
                                    ...
                                    className="h-8 w-10 text-xs text-center border border-gray-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-900 focus:ring-1 focus:ring-blue-500 p-0" 
                                  />
                                </form>
                              )}
                            </div>
                            ...
                            <div key={i} title={log ? `${log.value} ${tracker.unit||''}` : "No data"} className="w-10 flex justify-center items-center">
                              <div className={`flex h-8 w-8 items-center justify-center rounded-md border ${
                                log 
                                  ...
                                  : "bg-gray-50 dark:bg-zinc-800/50/50 border-gray-100 dark:border-zinc-800 text-gray-300"
                              }`}>
                    ...
                    <div className="absolute top-1/2 -translate-y-1/2 -right-6 hidden group-hover:block">
                      <form action={async () => { await deleteTracker(tracker.id); toast.success("Deleted"); }}>
                        <button type="submit" className="p-1.5 bg-white dark:bg-zinc-900 rounded-md border border-gray-200 dark:border-zinc-700 text-red-500 shadow-sm hover:bg-red-50 transition"><Trash2 className="h-3.5 w-3.5" /></button>
                      </form>
                    </div>
                  </div>
                );
        ...
        <div className="mt-8 border-t border-gray-100 dark:border-zinc-800 pt-6">
          {!showForm ? (
            <button onClick={() => setShowForm(true)} className="flex items-center gap-2 rounded-lg border-2 border-dashed border-gray-300 dark:border-zinc-600 px-4 py-3 text-sm font-medium text-gray-600 dark:text-zinc-400 hover:text-blue-600 hover:border-blue-600 hover:bg-blue-50/50 transition">
              <Plus className="h-4 w-4" /> Add New Habit
            </button>
          ) : (
            <form action={async (data) => { await createTracker(data); setShowForm(false); toast.success("Habit created"); }} className="max-w-xl space-y-4 bg-white dark:bg-zinc-900 p-5 rounded-xl border border-gray-200 dark:border-zinc-700 shadow-sm">
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-bold text-gray-900 dark:text-zinc-100">Create Habit</span>
                <button type="button" onClick={() => setShowForm(false)} className="text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:text-zinc-400"><XIcon className="h-5 w-5" /></button>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 sm:col-span-1 space-y-1.5">
                  <label className="text-xs font-semibold text-gray-700 dark:text-zinc-300">Habit Name</label>
                  <input type="text" name="title" required placeholder="e.g. Morning Run" className="w-full rounded-md border border-gray-300 dark:border-zinc-600 px-3 py-2 text-sm text-gray-900 dark:text-zinc-100 placeholder:text-gray-400 dark:placeholder:text-zinc-600 dark:text-zinc-500 focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none" />
                </div>
                <div className="col-span-2 sm:col-span-1 space-y-1.5">
                  <label className="text-xs font-semibold text-gray-700 dark:text-zinc-300">Type</label>
                  <select name="type" className="w-full rounded-md border border-gray-300 dark:border-zinc-600 px-3 py-2 text-sm text-gray-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input type="number" name="targetValue" placeholder="e.g. 30" className="w-full rounded-md border border-gray-300 dark:border-zinc-600 px-3 py-2 text-sm text-gray-900 dark:text-zinc-100 placeholder:text-gray-400 dark:placeholder:text-zinc-600 dark:text-zinc-500 focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-gray-700 dark:text-zinc-300">Unit (Optional)</label>
                  <input type="text" name="unit" placeholder="e.g. Mins, Pages" className="w-full rounded-md border border-gray-300 dark:border-zinc-600 px-3 py-2 text-sm text-gray-900 dark:text-zinc-100 placeholder:text-gray-400 dark:placeholder:text-zinc-600 dark:text-zinc-500 focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none" />
                </div>
              </div>
